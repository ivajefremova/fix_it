...
function CompareModal({ unis, onClose }: { unis: FavouriteUniversity[]; onClose: () => void }) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(24,24,49,0.72)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
      onClick={onClose}
    >
      <div
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose} style={{ color: 'rgba(24,24,49,0.7)', background: 'none', border: 'none', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `120px repeat(${unis.length}, 1fr)`, gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          {unis.map(u => (
            <div key={u.slug}>
              <Link href={`/universities/${u.slug}`} className="text-sm text-navy hover:underline" style={{ fontWeight: 500 }}>{u.name}</Link>
              <p className="text-xs mt-0.5" style={{ color: 'rgba(24,24,49,0.7)', fontWeight: 500 }}>{u.city}</p>
            </div>
          ))}
        </div>
        ...
        {rows.map(row => (
          ...
          >
            <p className="text-xs" style={{ color: 'rgba(24,24,49,0.7)', fontWeight: 500 }}>{row.label}</p>
            {unis.map(u => (
              <p key={u.slug} className="text-sm text-navy" style={{ fontWeight: 500 }}>
        ...
        <div
          style={{ display: 'grid', gridTemplateColumns: `120px repeat(${unis.length}, 1fr)`, gap: '1rem', padding: '0.75rem 0', borderTop: '1px solid #f0f2f5' }}
        >
          <p className="text-xs" style={{ color: 'rgba(24,24,49,0.7)', fontWeight: 500 }}>Fields</p>
          {unis.map(u => (
            <div key={u.slug} className="flex flex-col gap-1">
              {(u.tags ?? []).filter(t => !['english','bachelor','master','doctorate'].includes(t)).slice(0, 3).map(t => (
                <span key={t} className="flex items-center gap-1.5 text-sm" style={{ color: 'rgba(24,24,49,0.78)', fontWeight: 500 }}>
  ...
  if (!open) {
    return (
      <button
        ...
        style={{ color: initialNote ? 'rgba(24,24,49,0.78)' : 'rgba(24,24,49,0.82)', fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left' }}
      >
  ...
  return (
    <div className="mt-2">
      <textarea
        ...
        style={{ background: '#f8f9fb', border: '1px solid #e4ebf3', padding: '8px 10px', fontFamily: 'inherit', fontWeight: 500, color: '#181831' }}
      />
      <div className="flex gap-2 mt-1.5">
        <button
          ...
          style={{ background: '#181831', color: '#fff', fontWeight: 500, border: 'none', cursor: 'pointer' }}
        >
          {saving ? 'Saving…' : 'Save'}
        </button>
        <button
          ...
          style={{ background: '#f0f2f5', color: 'rgba(24,24,49,0.78)', fontWeight: 500, border: 'none', cursor: 'pointer' }}
        >
          Cancel
        </button>
      </div>
    ...
    return (
      <div className="text-center py-20">
        <div className="w-14 h-14 rounded-full mx-auto mb-4 flex items-center justify-center" style={{ background: '#f0f2f5' }}>
          <svg className="w-6 h-6" style={{ color: 'rgba(24,24,49,0.78)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
          </svg>
        </div>
        <p className="text-sm mb-1" style={{ color: 'rgba(24,24,49,0.75)', fontWeight: 500 }}>No saved universities yet.</p>
        <p className="text-sm mb-6" style={{ color: 'rgba(24,24,49,0.65)', fontWeight: 500 }}>Heart a university while browsing to save it here.</p>
        <Link
          href="/universities"
          className="inline-flex items-center gap-1.5 px-6 py-3 rounded-xl text-sm transition hover:opacity-90"
          style={{ background: '#51e74c', color: '#181831', fontWeight: 500 }}
        >
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          {([['recent', 'Recent'], ['az', 'A–Z'], ['country', 'Country']] as const).map(([v, label]) => (
            <button
              ...
              style={{
                ...
                color: sort === v ? '#fff' : 'rgba(24,24,49,0.78)',
        ...
        {compareIds.size >= 2 && (
          <button
            ...
            style={{ background: '#0c4d86', color: '#fff', fontWeight: 500, border: 'none', cursor: 'pointer' }}
          >
        ...
        {selected.size > 0 && (
          <button
            ...
            className="text-xs px-4 py-1.5 rounded-full transition hover:opacity-90"
            style={{ background: 'rgba(239,68,68,0.1)', color: '#ef4444', fontWeight: 500, border: 'none', cursor: 'pointer' }}
          >
      ...
      <div className="space-y-8">
        {byCountry.map(group => (
          <div key={group.slug}>
            <p className="text-xs uppercase tracking-widest mb-3" style={{ color: '#0c4d86', fontWeight: 700 }}>
              {group.country}
            </p>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-3 mb-2 flex-wrap">
                          <Link href={`/universities/${f.slug}`} className="text-base text-navy hover:underline" style={{ fontWeight: 500 }}>
                            {f.name}
                          </Link>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          {f.city && <span className="text-xs" style={{ color: 'rgba(24,24,49,0.7)', fontWeight: 500 }}>{f.city}</span>}
                          {f.city && f.type && <span style={{ color: '#e4ebf3', fontSize: 10 }}>·</span>}
                          {f.type && (
                            <span className="text-xs" style={{ color: 'rgba(24,24,49,0.72)', fontWeight: 500 }}>{f.type}</span>
                          )}
                          {f.tuition_range && (
                            <>
                              <span style={{ color: '#e4ebf3', fontSize: 10 }}>·</span>
                              <span className="text-xs" style={{ color: 'rgba(24,24,49,0.7)', fontWeight: 500 }}>{f.tuition_range}</span>
                            </>
                          )}
                        </div>
                        ...
                        {f.tags && f.tags.length > 0 && (
                          <div className="flex flex-col gap-1 mb-3">
                            {f.tags.filter(t => !['english','bachelor','master','doctorate'].includes(t)).slice(0, 4).map(tag => (
                              <span key={tag} className="flex items-center gap-2 text-sm" style={{ color: 'rgba(24,24,49,0.78)', fontWeight: 500 }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          color: isComparing ? '#0c4d86' : 'rgba(24,24,49,0.7)',
      ...
      <div className="mt-10 text-center">
        <Link
          ...
          style={{ color: '#0c4d86', fontWeight: 500 }}
        >
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-sm mb-1" style={{ color: 'rgba(24,24,49,0.75)', fontWeight: 500 }}>No saved guides yet.</p>
        <p className="text-xs mb-6" style={{ color: 'rgba(24,24,49,0.65)', fontWeight: 500 }}>Heart a guide on the services page to save it for later.</p>
        <Link
          href="/services"
          className="inline-flex items-center gap-1.5 px-6 py-3 rounded-xl text-sm transition hover:opacity-90"
          style={{ background: '#51e74c', color: '#181831', fontWeight: 500 }}
      ...
      <div className="space-y-3">
        {items.map(item => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-navy" style={{ fontWeight: 500 }}>
                {item.package_type === 'scholarship'
                  ? (item.item_name ?? 'Scholarship Guide')
                  : (PACKAGE_LABELS[item.package_type] ?? item.package_type)}
              </p>
              <p className="text-xs mt-0.5" style={{ color: 'rgba(24,24,49,0.7)', fontWeight: 500 }}>
                {item.country}
  ...
  return (
    <main className="min-h-screen" style={{ background: '#f8f9fb' }}>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        ...
        {/* Header */}
        <div className="bg-white rounded-2xl p-6 sm:p-8 mb-6 flex items-center gap-5 shadow-sm">
          <div className="w-14 h-14 rounded-full flex items-center justify-center text-lg flex-shrink-0" style={{ background: '#181831', color: '#ffffff', fontWeight: 500 }}>
            {initials}
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl text-navy" style={{ fontWeight: 600 }}>{name || 'Welcome'}</h1>
            <p className="text-sm mt-0.5 truncate" style={{ color: 'rgba(24,24,49,0.7)', fontWeight: 500 }}>{email}</p>
          </div>
          {userType && (
            <span className="hidden sm:block text-xs flex-shrink-0" style={{ color: 'rgba(24,24,49,0.72)', fontWeight: 500 }}>
              ...
              <div className="flex border-b" style={{ borderColor: '#eef0f3' }}>
                {TABS.map(t => (
                  <button
                    ...
                    style={{
                      color: tab === t ? '#0c4d86' : 'rgba(24,24,49,0.7)',
                      fontWeight: tab === t ? 600 : 500,
                      background: 'none', border: 'none', cursor: 'pointer',
                      borderBottom: tab === t ? '2px solid #0c4d86' : '2px solid transparent',
                      marginBottom: -1,
                    }}
                  >
                    {t}
                    {t === 'Universities' && favourites.length > 0 && (
                      <span className="ml-1 text-xs" style={{ color: 'rgba(24,24,49,0.65)', fontWeight: 500 }}>({favourites.length})</span>
                    )}
                    {t === 'Guides' && wishlist.length > 0 && (
                      <span className="ml-1 text-xs" style={{ color: 'rgba(24,24,49,0.65)', fontWeight: 500 }}>({wishlist.length})</span>
                    )}
                  </button>
                ))}
              </div>
